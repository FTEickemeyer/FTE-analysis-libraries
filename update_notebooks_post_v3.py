"""Patch notebooks for API fixes discovered after running update_notebooks_post_v2.py.

Renames covered here:

  1. .load(..., FN=...) → .load(..., filepath=...)
     (XYData.load() / MXYData.load() kwarg renamed from FN to filepath)
     Matched only when preceded by a comma (keyword argument position).
     [review] hits should be verified to be .load() calls, not other functions.

Add new entries to WORD_REPLACEMENTS or REGEX_REPLACEMENTS as further issues
are discovered.

Usage:
    python update_notebooks_post_v3.py           # dry run (shows what would change)
    python update_notebooks_post_v3.py --apply   # apply (creates .bak_post_v3 backups)
"""
...
WORD_REPLACEMENTS: list[tuple[str, str]] = [
    # Add word-boundary-safe renames here.
]

# Raw regex replacements — used when word boundaries don't fit.
...
def migrate_notebook(path: Path, dry_run: bool) -> tuple[bool, list[str]]:
    ...
    if modified and not dry_run:
        backup = path.with_suffix(".ipynb.bak_post_v3")
...
def migrate_pyfile(path: Path, dry_run: bool) -> tuple[bool, list[str]]:
    ...
    if not dry_run:
        backup = path.with_suffix(".py.bak_post_v3")
        backup.write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    print(f"Mode: {'DRY RUN - no files written' if dry_run else 'APPLY - writing changes + .bak_post_v3 backups'}\n")
    ...
    if dry_run and changed_files:
        print("\nRe-run with --apply to write changes (backups saved as .bak_post_v3).")
    if any("[review" in c for _, changes in changed_files for c in changes):
        print("\n[!] Some changes are marked [review] — verify FN= replacements are inside .load() calls.")
