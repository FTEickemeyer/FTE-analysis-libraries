...
def det_c_V_from_conc_and_electrolyte_details(conc: Any, weight_pc_V_start: Any=6.11, weight_pc_SO4: Any=28, density: Any= 1.35, show: bool=False) -> Any:
    ...
    weight_pc_V = p[0]
    if show:
        print(f'Vanadium: {weight_pc_V:.3f} wt%')
    c_V, c_SO4 = conc_V_SO4(weight_pc_V=weight_pc_V, weight_pc_SO4=weight_pc_SO4, density=density, show=show)
    return c_V, c_SO4
...
def combine_both_steps(df_charge_step1: Any, df_charge_step2: Any, df_voltage: Any, df_current: Any, show: bool=False) -> Any:
    ...
    if show:
        ...
        axl.set_title('Charging protocol')
        ...
        ax.set_title('Total charge versus time')
...
def det_time_and_charge_SOC0_and_SOC1_from_charging_protocol(df_charge: Any, conc: Any, Vol: float, show: bool=False) -> Any:
    ...
    if show:
        print('Calculations from charging protocol only:')
    ...
    # Calculate the time that this charge is reached
    idx_SOC1 = np.argmin(np.abs(df_charge-(charge_SOC1_calc+charge_SOC0_calc)))
    time_to_reach_SOC1 = df_charge.index[idx_SOC1]
    charge_SOC1 = df_charge.iloc[idx_SOC1]
    if show:
        if charge_SOC1 < charge_SOC1_calc:
            print('This charge is not reached in the charging protocol!')
            print(f'Maximum charge ({charge_SOC1:.3f} Ah) reached after {time_to_reach_SOC1} s')
        else:
...
def det_df_conc_from_charging(c_V: np.ndarray, Vol: float, av_ox_state_initial: Any, df_charge: Any, show: bool=False) -> Any:
    ...
    df_charge_avox['n_redox'] = df_charge_avox['Charge (Ah)']*3600 / (F * c_V * Vol)
    df_charge_avox['av_ox_state_pos'] = av_ox_state_initial + df_charge_avox['n_redox']
    df_charge_avox['av_ox_state_neg'] = av_ox_state_initial - df_charge_avox['n_redox']
    #display(df_charge_avox)
    
    df_conc = df_charge.copy().to_frame()
    df_conc = df_conc.drop('Charge (Ah)', axis=1)
    df_conc['charging_V3_pos'] = (4 - df_charge_avox['av_ox_state_pos']) * c_V
    df_conc['charging_V4_pos'] = df_charge_avox['av_ox_state_pos'].apply(lambda x : x - 3 if (x >= 3 and x <= 4) else (5 - x if (x > 4) else 0)) * c_V
    df_conc['charging_V5_pos'] = (df_charge_avox['av_ox_state_pos'] - 4) * c_V
    df_conc['charging_V4_neg'] = (df_charge_avox['av_ox_state_neg'] - 3) * c_V
...
def upload_charging_protocol_BKPrecision(fp: Any, voltage_limit_step1: Any, correct_time: Any=True, potential_measurement_system: Any | None=None, FN_negolyte_potential: str | None=None, data_dir: str | None=None) -> Any:
    ...
    df_raw = df_raw[['Time (s)', 'Voltage - Voltage', 'Current - Current']].set_index('Time (s)')
    df_raw = df_raw.rename(columns={'Voltage - Voltage': 'Voltage(V)', 'Current - Current': 'Current(A)'})
    df = df_raw.iloc[::30,:]
    df_charging_voltage = df['Voltage(V)']
    df_charging_current = df['Current(A)']
    #df_charging_voltage = df[['Samples - Voltage', 'Voltage - Voltage']].rename(columns={'Samples - Voltage':'Time (s)', 'Voltage - Voltage': 'Voltage(V)'}).set_index('Time (s)').squeeze()
    #df_charging_current = df[['Samples - Current', 'Current - Current']].rename(columns={'Samples - Current':'Time (s)', 'Current - Current': 'Current(A)'}).set_index('Time (s)').squeeze()
    #Calculate Charge from current   
    #df_diffAh = df_charging_current.rolling(2).apply(lambda g: integrate.trapezoid(g.values, x=g.index.values))
    df_diffAh = df_charging_current.rolling(2).apply(lambda g: (g.values[-1]+g.values[0])/2*(g.index.values[-1]-g.index.values[0]))
    df_charge = df_diffAh.cumsum()/3600
    df_charge.iloc[0] = 0.0
    df_charge.rename('Charge (Ah)', inplace=True)
    idx = np.where(df_raw['Voltage(V)'].values == voltage_limit_step1)[0][0]
    first_charging_step_end_s = df_raw.index.values[idx]
...
def plot_charging_protocol(df_charge: Any, df_voltage: Any, df_current: Any, first_charging_step_end_s: Any) -> Any:
    ...
    axr = axl.twinx()
    curve2,  = axr.plot(df_current.index, df_current, color='tab:orange', linewidth=5, label='Current')
    axl.axvline(first_charging_step_end_s, color='black')
    #axl.set_xlim(250, 1100)
    axl.set_ylim(1.0, 1.7)
    axr.set_ylim(0.0, 2.5)
    # Combine legends from both axes
    ...
    ax = ax[1]
    ax.plot(df_charge.index, df_charge, color='tab:blue', linewidth=5, label='Charge')
    ax.axvline(first_charging_step_end_s, color='black')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Charge (Ah)')
    #axl.set_ylim((1.3, 1.65))
    #axl.set_xlim((0, 17500))
...
def upload_potential_difference_measurement(potential_measurement_system: Any, 
    ...
    if potential_measurement_system == 'biologic':
        ...
        raw_data = gen.df_interpolate(raw_data, new_times_s).squeeze()
        if raw_data.values[-1] < 0:
            raw_data *= -1
        raw_data = raw_data.clip(lower=0)
        if show:
            # Plot
            fig, ax = plt.subplots()
            raw_data.plot(ax=ax)
            last_pot_diff = raw_data.values[-1]
            ax.axhline(last_pot_diff, color='black')
            print(f'Last potential difference: {last_pot_diff:.3f} V')
            ax.set_xlabel('Time (s)')
            ax.set_ylabel('Voltage (V)')
            plt.legend(['Potential difference Posolyte-Negolyte'])
            plt.show()
        ...
        raw_data.set_index('Time (s)', inplace=True)
        raw_data = raw_data[['V1 (V)']]
        #Reduce the size to n_times time steps
        new_times_s = np.linspace(0, raw_data.index[-1], no_times)
        raw_data = gen.df_interpolate(raw_data, new_times_s)
...
def det_times_and_charge_SOC0_from_potential_difference(df_charge: Any, Pdiff: Any, delta_SOC_posneg: Any='positive', avox_greater_3p5: Any | None=None, xlim: Any | None=None, threshold_divisor: Any=3, show: bool=False) -> Any:
    ...
    #Determine the times of the two peaks
    df = Pdiff.loc[xlim[0]:xlim[1]].diff().iloc[1:]
    peak_max = df.max()
    time = df.index  # If time is the index, otherwise use df['Time']
    voltage = df.values
    # Find peaks
    ...
    if delta_SOC_posneg == 'positive':
        ...
        tV3_neg = min(peak_times)
        tV4_pos = max(peak_times)
    if show:
        print(f'Time when all V3 is oxidized to V4 in positive electrolyte: tV4_pos = {tV4_pos:.0f} s')
        print(f'Time when all V4 is reduced to V3 in negative electrolyte: tV3_neg = {tV3_neg:.0f} s')
    ...
    if show:
        ...
        fig, ax = plt.subplots()
        ax.plot(dPdiff.index, dPdiff.values, label='Derivative')
        ax.set_xlim(xlim)
        #ax.set_ylim(0, 2.5)
        ax.axvline(tV3_neg)
        ax.axvline(tV4_pos)
...
def upload_potential_measurement(potential_measurement_system: Any,
    ...
    if potential_measurement_system == 'biologic':
        ...
        # Calibrate
        Ppos_raw = exp_pos + refCellCalibration_pos
        Pneg_raw = exp_neg + refCellCalibration_neg
        if show:
            # Plot
            fig, ax = plt.subplots()
            Ppos_raw.plot(ax=ax)
        ...
        #print(Praw.describe())
        # Calibrate
        Ppos_raw = Praw['V2 (V)'] + refCellCalibration_pos
        Pneg_raw = Praw['V1 (V)'] + refCellCalibration_neg
        if show:
            # Plot
            fig, ax = plt.subplots()
            Ppos_raw.plot(ax=ax)
...
def det_times_and_charge_SOC0_from_potential(df_charge: Any, Ppos: Any, Pneg: Any, xlim: Any | None=None, show: bool=False) -> Any:
    ...
    if show:
        ...
        fig, ax = plt.subplots()
        ax.plot(dPpos.index, dPpos.values, label='Derivative posolyte')
        ax.plot(dPneg.index, dPneg.values, label='Derivative negolyte')
        ax.set_xlim(xlim)
        #ax.set_ylim(0, 2.5)
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('dP/dt (V/s)')
        plt.legend()
        plt.show()
...
def det_initial_av_ox_state_from_potential(df_charge: Any, tV4_pos: Any, tV3_neg: Any, charge_tV4_pos: Any, charge_tV3_neg: Any, Vol_neg: Any, Vol_pos: Any, c_V: np.ndarray, show: bool=False) -> Any:
    ...
    n_redox_pos = 1/q*charge_tV4_pos*3600 / (c_V * Vol_pos * Avogadro_const)
    if show:
        print(f'The number of redox reactions per vanadium ion in the negolyte carried out until oxidation state 3.0 is reached: {n_redox_neg:.3f}')
        print(f'The number of redox reactions per vanadium ion in the posolyte carried out until oxidation state 4.0 is reached: {n_redox_pos:.3f}')
        
        
    #Caluclate number of redox reactions per vanadium ion carried out during t_diff
    #1. step: Calculate charge difference in Ah
    charge_difference = charge_tV3_neg - charge_tV4_pos
    #2. step: calculate n_redox for negative and positive electrolyte
    n_redox_neg = 1/q*charge_difference*3600 / (c_V * Vol_neg * Avogadro_const)
    n_redox_pos = 1/q*charge_difference*3600 / (c_V * Vol_pos * Avogadro_const)
    if show:
        print(f'The number of redox reactions per vanadium ion in the negolyte carried out during time difference: {n_redox_neg:.3f}')
        print(f'The number of redox reactions per vanadium ion in the posolyte carried out during time difference: {n_redox_pos:.3f}')
    #At tV4_pos the posolyte is at an oxidation state 4.0 and at tV3_neg the negolyte is at an oxidation state 3.0 
    #At t = 0 the posolyte had oxidation state av_ox_state_pos_t0
    #av_ox_state_pos_t0 = 
    
    #Average initial oxidation state if posolyte and negolyte were the same at t = 0
    #av_ox_state_t0 = 
    
    #If posolyte and negolyte had the same oxidation state at the beginning, then the product of concentration and volume can be calculated
    #The following relation must hold: 
    #4 - n_redox_pos = 3 + n_redox_neg, from this it follows
    #new_c_V = 3600/F*(charge_tV4_pos/Vol_pos + charge_tV3_neg/Vol_neg)
...
def det_conc_and_initial_oxidation_state(charge_tV4_pos: Any, charge_tV3_neg: Any, Vol_pos: Any, Vol_neg: Any, show: bool=False) -> Any:
    ...
    noxi = 4 - delta_n_pos_tV4pos_times_conc/conc
    noxi_alternative = 3 + delta_n_neg_tV3neg_times_conc/conc
    if np.abs(noxi-noxi_alternative) > 0.001:
        print(f'Initial oxidation state not unique. noxi = 4 - delta_n_pos_tV4pos_times_conc/conc = {noxi}, noxi_alternative = 3 + delta_n_neg_tV3neg_times_conc/conc = {noxi_alternative}')
    if show:
        print(f'Calculated vanadium concentration: conc = {conc:.3f} M')
        print(f'Initial oxidation state: {noxi:.3f}')
        print(f'Change in oxidation state of posolyte until V4 reached: {delta_n_pos_tV4pos_times_conc/conc:.3f}')
...
def fit_potentials(c_V: np.ndarray, c_SO4: Any, Ppos: Any, Pneg: Any, df_charge: Any, Ppos_charge: Any, Pneg_charge: Any, E_start_exp: Any, Vol: float, use_proton_concentration: bool, T: float=gen.T_RT, fit_conc: Any=True, show_details: bool=True, redox_potential_guess_V_vs_SHE: Any=dict(E0_V2X3=-0.25, E0_V3X4=0.394, E0_V4X5=1.00)) -> Any:
    ...
    if fit_conc:
        ...
        p0 = [E0_V2X3, E0_V3X4, E0_V4X5]
        bounds = (
                  [E0_V2X3-0.2, E0_V3X4-0.2, E0_V4X5-0.2],
                  [E0_V2X3+0.2, E0_V3X4+0.2, E0_V4X5+0.2]
                  )
...
def calculate_fit(c_V_start: Any, c_SO4_start: Any, 
    ...
    if do_recalculation:
        ...
        c_V, E0s, df_fits, conc_pos, conc_neg = fit_potentials(c_V, c_SO4, Ppos, Pneg, df_charge, Ppos_charge, Pneg_charge, E_start_exp, Vol, use_proton_concentration, T=T, fit_conc=fit_conc, show_details=show_details, redox_potential_guess_V_vs_SHE=redox_potential_guess_V_vs_SHE)
        if show_details:
...
def plot_complete_dataset(fit_data: np.ndarray, Ppos: Any, Pneg: Any, 
    ...
    if plot_t_V34:
        ax.vlines(t_V3_neg,min(min(Ppos), min(Pneg)), max(max(Ppos), max(Pneg)), color='tab:orange', linestyle='--', linewidth=1.0, label='Time for negolyte to reach V3+')
        ax.vlines(t_V4_pos,min(min(Ppos), min(Pneg)), max(max(Ppos), max(Pneg)), color='tab:blue', linestyle='--', linewidth=1.0, label='Time for posolyte to reach V4+')
    if plot_time_to_reach_SOC0:
    ...
    if plot_redox_potentials:
        ax.hlines([E0_V2X3, E0_V3X4, E0_V4X5], min(Ppos.index), max(Ppos.index), color='tab:green', linewidth=1.0, linestyle='--')
    ax.set_xlim(xlim)
    if ylim is not None:
        ax.set_ylim(ylim)
    ax.set_xlabel('Time (s)')
...
def load_ref_spec(ref_spec_dir: str, nm_array: Any, electrolyte_system: Any='V-V', xlim: Any | None=None) -> Any:
    ...
    if electrolyte_system == 'V-V':
        ref_spec_negred = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_V2.csv'), index_col=0))
        ref_spec_negox = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_V3.csv'), index_col=0))
        ref_spec_posred = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_V4.csv'), index_col=0))
        ref_spec_posox = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_V5.csv'), index_col=0))
    elif electrolyte_system == 'Fe-V':
        ref_spec_negred = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_Fe2V2.csv'), index_col=0))
        ref_spec_negox = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_Fe2V3.csv'), index_col=0))
        ref_spec_posred = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_Fe2V4.csv'), index_col=0))
        ref_spec_posox = df_to_series(pd.read_csv(os.path.join(ref_spec_dir, 'spec_Fe3V4.csv'), index_col=0))
    all_ref_spec = [ref_spec_negred, ref_spec_negox, ref_spec_posred, ref_spec_posox]
    if xlim is None:
        return tuple(all_ref_spec)
    else:
...
def plot_ref_spectra(ref_spec_negred: Any, ref_spec_negox: Any, ref_spec_posred: Any, ref_spec_posox: Any, electrolyte_system: Any='V-V', xlim: Any | None= None, ylim: Any | None= None) -> Any:
    ...
    if electrolyte_system == 'V-V':
        ...
        label_negred = 'Fe(II)-V(II)'
        label_negox = 'Fe(II)-V(III)'
        label_posred = 'Fe(II)-V(IV)'
        label_posox = 'Fe(III)-V(IV)'
    ...
    plot_df(ax, ref_spec_negox, color='green', label=label_negox)
    plot_df(ax, ref_spec_posred, color='blue', label=label_posred)
    plot_df(ax, ref_spec_posox, color='orange', label=label_posox)
...
def load_UVVIS_raw_data(data_dir: str, nm_array: Any, xlim: Any | None=None) -> Any:
    ...
    def read_file(data_dir: str, filepath: str, xlim: Any) -> Any:
        ...
        if df.columns.values[0] == 'nm':
            ...
            df.set_index('Wavelength (nm)', inplace=True)
        elif df.columns.values[0] == 'Wavelength (nm)':
            df.set_index('Wavelength (nm)', inplace=True)
        elif df.columns.values[0] == 'delta_t':
            df.rename(columns={'delta_t': 'Wavelength (nm)'}, inplace=True)
            df.set_index('Wavelength (nm)', inplace=True)
            df = df.transpose()
            df.index = df.index.astype('float64')
        if not np.array_equal(df.index.values, nm_array):
            df = gen.df_interpolate(df, nm_array)
        if xlim is not None:
            return df.loc[xlim[0]:xlim[1]]
        else:
    ...
    df_dark = read_file(data_dir, 'dark.csv', xlim)
    df_I0_c1 = read_file(data_dir, 'I0_c1.csv', xlim)
    df_I0_c2 = read_file(data_dir, 'I0_c2.csv', xlim)
    ...
    def calc_absorbance(df_IT: Any, df_I0: Any, df_dark: Any, xlim: Any) -> Any:
        ...
        I0 = np.where(I0<=0, 1e-10, I0)
        df_T = df_IT.sub(dark, axis='index').div(I0, axis='index')
        #Make sure that T never becomes <= 0 because it appears in the log10 statement
        df_T[df_T<1e-4] = 1e-4
        if xlim is not None:
            return -np.log10(df_T).loc[xlim[0]:xlim[1], :]
        else:
            return -np.log10(df_T)

    df_cuv_pos = calc_absorbance(df_IT_c1, df_I0_c1, df_dark, xlim)
    df_cuv_neg = calc_absorbance(df_IT_c2, df_I0_c2, df_dark, xlim)
...
def plot_UVVIS_data_SOC(df_cuv_pos_raw: Any, df_cuv_neg_raw: Any, tV4_pos: Any, tV3_neg: Any, 
    ...
    time_array_pos = np.array([int(round(tV4_pos*i/no_curves)) for i in range(no_curves+1)])
    plot_UVVIS_data(df_cuv_pos_raw, df_cuv_neg_raw, ylabel=ylabel, time_array_pos=time_array_pos, time_array_neg=time_array_neg,
                        xlim_cuv_pos=xlim_cuv_pos, xlim_cuv_neg=xlim_cuv_neg, ylim_cuv_pos=ylim_cuv_pos, ylim_cuv_neg=ylim_cuv_neg)
    #Plot absorbance curves from SOC 0% until SOC 100%
    time_array_neg = np.array([int(tV3_neg) + int(round((df_cuv_neg_raw.columns[-1]-tV3_neg)*i/no_curves)) for i in range(no_curves+1)])
    time_array_pos = np.array([int(tV4_pos) + int(round((df_cuv_pos_raw.columns[-1]-tV4_pos)*i/no_curves)) for i in range(no_curves+1)])
    plot_UVVIS_data(df_cuv_pos_raw, df_cuv_neg_raw, ylabel=ylabel, time_array_pos=time_array_pos, time_array_neg=time_array_neg,
                        xlim_cuv_pos=xlim_cuv_pos, xlim_cuv_neg=xlim_cuv_neg, ylim_cuv_pos=ylim_cuv_pos, ylim_cuv_neg=ylim_cuv_neg)
...
def UVVIS_to_molar_extinction_coefficient(df_cuv_pos_raw: Any, df_cuv_neg_raw: Any, conc: Any, pathlength_cuv_neg: Any, scaling_factor_cuv_pos: Any, offset_cuv_pos: Any, show_details: bool=False) -> Any:
    ...
    if show_details:
        print(f'Pathlength positive cuvette: {pathlength_cuv_pos:.5f} cm')
        print(f'Pathlength negative cuvette: {pathlength_cuv_neg:.5f} cm')
...
def UVVIS_calculate_fit_for_all(df: Any, conc: Any, time_switch_fitspectra: Any, fit_spectra: Any, time_from: Any, time_to: Any, nm_lim_fit: Any) -> Any:
    ...
    for idx, time in enumerate(df.columns):
        if time >= time_from and time <= time_to:
            ...
            if time < time_switch_fitspectra:
                fit_spectrum_Vr = fit_spectra[0].loc[nm_from:nm_to]
                fit_spectrum_Vo = fit_spectra[1].loc[nm_from:nm_to]
            else:
                fit_spectrum_Vr = fit_spectra[2].loc[nm_from:nm_to]
                fit_spectrum_Vo = fit_spectra[3].loc[nm_from:nm_to]
            fit_spectrum_Vr_list.append(fit_spectrum_Vr.values)
            fit_spectrum_Vo_list.append(fit_spectrum_Vo.values)

            target_spectrum = df.loc[nm_from:nm_to, time]
    ...
    wavelengths = df.loc[nm_from:nm_to].index
    ...
    #Calculate concentrations (normalized to conc)
    cr_array = ar_array * conc/(ao_array+ar_array)
    co_array = conc - cr_array
    
    data = {'Measurement': df_target, 'Fit': df_fit, 'ar': ar_array, 'ao': ao_array, 'cr': cr_array, 'co': co_array, 'Time (s)': time_list, 'Spectra_Vr': df_fit_spectra_Vr, 'Spectra_Vo': df_fit_spectra_Vo}
    return data
...
def animate_UVVIS(data_cuv_pos: Any, data_cuv_neg: Any, conc: Any, 
    ...
    def animate(i: float) -> Any:
        ...
        def set_data_conc(line1: Any, line2: Any, SOC_label: Any, data_cuv: Any) -> Any:
            (df_target, df_fit, ar_array, ao_array, cr_array, co_array, time_list, df_fit_spectra_Vr, df_fit_spectra_Vo) = tuple(data_cuv.values())
            idx = i
            if idx >= len(time_list)-1:
                idx = len(time_list)-1
