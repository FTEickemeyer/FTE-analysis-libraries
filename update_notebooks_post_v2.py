"""Patch notebooks for API fixes discovered after running update_notebooks_post_v1.py.

Renames covered here:

  1. all_values_greater_min(min=...) → all_values_greater_min(min_val=...)
     (MXYData / XYData — kwarg renamed to avoid shadowing Python builtin)

Add new entries to WORD_REPLACEMENTS or REGEX_REPLACEMENTS as further issues
are discovered.

Usage:
    python update_notebooks_post_v2.py           # dry run (shows what would change)
    python update_notebooks_post_v2.py --apply   # apply (creates .bak_post_v2 backups)
"""
...
# Raw regex replacements — used when word boundaries don't fit (e.g. keyword
# argument renames, method-call patterns).  Always shown in output for review.
...
def migrate_notebook(path: Path, dry_run: bool) -> tuple[bool, list[str]]:
    ...
    if modified and not dry_run:
        backup = path.with_suffix(".ipynb.bak_post_v2")
...
def migrate_pyfile(path: Path, dry_run: bool) -> tuple[bool, list[str]]:
    ...
    if not dry_run:
        backup = path.with_suffix(".py.bak_post_v2")
        backup.write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    print(f"Mode: {'DRY RUN - no files written' if dry_run else 'APPLY - writing changes + .bak_post_v2 backups'}\n")
    ...
    if dry_run and changed_files:
        print("\nRe-run with --apply to write changes (backups saved as .bak_post_v2).")
