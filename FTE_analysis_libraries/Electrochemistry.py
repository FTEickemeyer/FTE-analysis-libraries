        ...
        if Z_fit_list is None:
            ...
            for f, Z, f_fit, Z_fit, label, color, fmt in zip(f_list, Z_list, f_fit_list, Z_fit_list, label_list, colors, fmts):
                if not f_range is None:
                    ...
                    plot_nyquist(ax, Z[ra], fmt=fmt, color = color, label = label)
                    plot_nyquist(ax, Z_fit[ra_fit], fmt='-', color = color, label = label+' (fit)')
                else:
                    plot_nyquist(ax, Z, fmt=fmt, color = color, label = label)
        ...
        if Z_fit_list is None:
            ...
            for f, Z, f_fit, Z_fit, label, color, fmt in zip(f_list, Z_list, f_fit_list, Z_fit_list, label_list, colors, fmts):
                if not f_range is None:
                    ...
                    ra_fit = idx_range(f_fit, left = f_range[0], right = f_range[1])
                    plot_bode([ax[0], ax[1]], f[ra], Z[ra], fmt=fmt, color = color, label = label)
                    plot_bode([ax[0], ax[1]], f_fit[ra_fit], Z_fit[ra_fit], fmt='-', label = label+' (fit)')
                else:
                    plot_bode([ax[0], ax[1]], f, Z, fmt=fmt, color = color, label = label)
                    plot_bode([ax[0], ax[1]], f_fit, Z_fit, fmt='-', label = label+' (fit)')
